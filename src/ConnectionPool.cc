...
#include <thread>
#include <iostream>
ConnectionPool::ConnectionPool(size_t max_size, std::chrono::seconds max_idle_time)
...
bool ConnectionPool::CreateConnection(const std::string& ip, uint16_t port) {
...
std::shared_ptr<Connection> ConnectionPool::GetConnection(const std::string& ip, uint16_t port) {
    ...
    std::cout << "Failed to get connection from pool";
    return nullptr;
}
